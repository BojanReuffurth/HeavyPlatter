...
struct SuggestionsView: View {
    ...
    private var suggestionsGrid: some View {
        ...
        .simultaneousGesture(
            DragGesture(minimumDistance: 5)
                .onChanged { _ in
                    guard !flippedIds.isEmpty else { return }
    ...
    private func cardBack(_ rec: SuggestedRecord) -> some View {
        VStack(spacing: 0) {
            // Top: flip-back | provider | details
            HStack {
                Button {
...
// MARK: – Cover image loader (URLSession + iTunes fallback)

private struct SuggestionCoverImage: View {
    let rec: SuggestedRecord
    ...
    var body: some View {
        ...
        .task(id: rec.id) {
            attempted = false
            uiImage   = await loadBestCover(for: rec)
            attempted = true
        }
    }
}

private func loadBestCover(for rec: SuggestedRecord) async -> UIImage? {
    // Try Discogs high-res first, then thumb
    let candidates = [rec.coverImageURL, rec.coverURL].filter { !$0.isEmpty }
    for urlStr in candidates {
        if let img = await fetchImage(from: urlStr) { return img }
    }
    // iTunes fallback (cover art CDN is more reliable)
    let iTunesURLs = await iTunesClient.liveValue.fetchCoverURLs(rec.artist, rec.album)
    for urlStr in iTunesURLs.prefix(2) {
        if let img = await fetchImage(from: urlStr) { return img }
    }
    return nil
}

private func fetchImage(from urlStr: String) async -> UIImage? {
...
struct SuggestionDetailSheet: View {
    ...
    // MARK: Tracklist (iTunes with audio previews)
